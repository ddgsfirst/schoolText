...
def list_students(db: Session = Depends(get_client_db)):
...
def get_student(student_id: int, db: Session = Depends(get_client_db)):
    """
...
def delete_student(student_id: int, db: Session = Depends(get_client_db)):
